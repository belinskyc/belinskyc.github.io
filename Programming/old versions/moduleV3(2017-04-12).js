...
/* Whenever an in-page jump is made, a return link will appear in the
	element that return you to the previous positioin */
	...
	$("a").each(function () // for all anchors
	{
		...
		if (href && href[0] == "#")  // check for a hash tag
		{
			...
			$(this).click(function () 
			{
				...
				{
					/*****
					Need to incorporate this with the scrollToElement function --
					perhaps when we convert this whole function to javascript only					
					******
...
function createInPageLinks()
{
	...
	// get all [a] in the page
	anchorElements = document.getElementsByTagName("a");
	
	for(i=0; i<anchorElements.length; i++)  // for each [a]
	{
		// only care about the [a] that have an href with a hash link "#"
		if(anchorElements[i].href && anchorElements[i].href.includes("#"))
		{
			// get the position of the # in the href
			hashIndex = anchorElements[i].href.indexOf("#") + 1;  // don't need the '#', hence the +1
			// slice the href location after the '#' -- leave just the ID of the linkTo element
			hashTagID = anchorElements[i].href.slice(hashIndex);
			
			// remove the href, using onclick instead
			anchorElements[i].removeAttribute("href");
			
			// add linkTo class
			anchorElements[i].classList.add("linkTo");
			
			// need to check if this is a valid element
			linkToElement = document.getElementById(hashTagID);
		
			anchorElements[i].onclick = scrollToElement(linkToElement.id);
		}
...
function addCodeTags(elementType)
{
	...
	/* count the number of address or H6 tags
	...
	for(i=0; i<numCodeTags; i++)
	{
		...
		{ 
			...
			for(j=0; j<numLines; j++)
			{
				...
				//codeText[j] = $(codeLines[i]).html().split("<br>")[j];
...
function scrollToElement(elementID)
{
	...
	return function() 
	{
		...
		{
			// get the iframe location from top of page 
			iframeOffset = window.parent.document.getElementsByTagName("iframe")[0].offsetTop;
			// get the amount of vertical space the header take
			headerHeight = window.parent.document.getElementById("d2l_minibar").offsetHeight;
